...
export default function Hero({ show }) {
  ...
  useEffect(() => {
    if (show) {
      const t = setTimeout(() => setVisible(true), 50); // fade subito dopo splash
  ...
  return (
    <section style={section}>
      <h1 style={{ fontSize: "3rem", fontWeight: 800, marginBottom: 16 }}>
        Trova il tuo prossimo viaggio con{" "}
        <span style={{ color: "#f97316" }}>ItinerAI</span>
      </h1>
      <p style={{ marginBottom: 32, fontSize: "1.2rem" }}>
        Cerca voli, hotel ed esperienze in pochi clic
      </p>
